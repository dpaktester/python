...
wb = openpyxl.load_workbook("LupiniPadDataValiadtions_09252019.xlsx")
dataSheet1 = wb["National Aggregate"]
dataSheet2= wb["Prescriber Plan Level"]
channelNotIncludedSuprax=["Cash","HIX","Mail Order","PBM","VA","Other Third Party"]
channelNotIncludedSolosec=["Cash","HIX","Mail Order","PBM","VA","Other Third Party","Medicare"]
...
def DifferenceofMattyinRawDbandIpadDB():
    ...
    RawDBQuery= """ SELECT A.SRA2 DEA_NO,A.ME_NO PRSC_ID,A.L_NM L_NM, A.F_NM F_NM,C.ENTITY_ID,C.CHANNEL_ID, B.THERA_ID,B.DRUG_BRD_ID,F.DRUG_BRAND_NM,
                    ...
                    FROM [B1-123].DI_MASTER.dbo.TMP_ENTITY_IMS_XREF X WHERE X.CHANNEL_ID NOT IN (275,283,284))C
    ...
    DiffernceBetweenRawDBAndReportDb="""    SELECT DISTINCT A.PRSC_ID,B.EntityID,A.RawChannel,B.EntityName,B.Channel iPadChannel,
                                            ...
                                            WHEN A.CHANNEL_ID IN ('278','279','280') THEN 'Medicare' END AS RawChannel,
                                            CASE 
    ...
    if(DifferenceofBothiPadandRawDb.__len__()==0):
      ...
      print("Test Case Failed for below Prescriebrs","\n")
      for each in range(DifferenceofBothiPadandRawDb.__len__()):
        tuple1 = DifferenceofBothiPadandRawDb[each]
        if (count == 500):
            break
        #print(tuple1)
        count = count + 1
        print(str(tuple1[0]) + "|" + str(tuple1[1]) + "|" + str(tuple1[2]) + "|" + str(tuple1[3]) + "|" + str(tuple1[4])+"\n")
      return 1
...
def Verify_Channel_in_NationalAggregate():
    ...
    if difference_of_channel==set():
     count=0
        ...
        count=1
    if(count==0):
        print("Channels are matching agaisnt the Report for Prescriber Plan Level tab")
        return 0
    else:
        print("TestCase Failed ","\n","Difference of Channels are in for Prescriber Plan Level tab", difference_of_channel)
        return 1
...
def Verify_Channel_in__Prescriber_Plan_Level_Sheet():
    channelSet__in__File_list = ['Commercial', 'Medicare', 'Medicaid', 'TRICARE']
...
def Verify_BlankEntity__in_Prescriber_Plan_Level_Sheet():
    ...
    if(count==0):
            ...
            print("There is a Blank Rows in Entity Name Field of Prescriber Plan Level Sheet for Prescriber ID ",blankEntitiesinfo)
            return 1
...
def Verify_Blank_Precriber_in_Prescriber_Plan_Level_Sheet():
    ...
    if(count==0):
            ...
            print("There is a Blank Rows in Prescriber ME Number Field of Prescriber Plan Level Sheet for Prescriber ID ","\n",blankPrescriberMeNos,"\n")
            return 1
...
def verify__Antara_in_Prescriber_Plan_Level_sheet():
    ...
    if (count == 0):
        ...
        print("There is Antara in the Static field of Prescriber Plan Level Sheet", "\n", Static_Information_Prescriber_Plan_Level_sheet, "\n")
        return 1