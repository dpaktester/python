...
defaultRegions = ['43000', '44000', '99000']
...
WomensHealthCare_TQTY_drugs = ['Cleocin Vaginal', 'Cleocin Vaginal Ovules', 'clindamycin phosphate vaginal cream',
                               'Clindesse', 'Flagyl', 'Metrogel-Vaginal', 'metronidazole', 'metronidazole vaginal',
...
def CheckRegions_For_Report():
    regionQuery1 = "SELECT DISTINCT Region FROM LupinAllPrescribersReportData"
    regionQueryoutput = cursor.execute(regionQuery1)
    region = regionQueryoutput.fetchall()
    regionlist = []
    for i in range(list(region).__len__()):
        regionlist.append(tuple(region[i]).__getitem__(0))
        Missing_Regions_IN_All_PrescriberReport = set(regionlist) - set(defaultRegions)
        if (Missing_Regions_IN_All_PrescriberReport.__len__()) == 0:
            print("The region is matching aganist The Requiement")
        else:
            print("The region is not matching against The Reqirement")
            print(Missing_Regions_IN_All_PrescriberReport)

    return Missing_Regions_IN_All_PrescriberReport.__len__()
...
def RAWDBMattySupraxCapTabManagedMedicaid():
        ...
        rawDbQuery1= """    Select regionid,BUID,ME_NO,SRA2 IMSDR_Key,F_NM,L_NM,c.drug_name DrugName,Case when F.IMS_NUM IS NULL Then 'N' else 'Y' End as PDRP,
                            ...
                            Inner join [B1-124].M1B_LUP.dbo.PrescriberAlignment T on A.SRA2=T.IMSID  and BUID is not null                            
                            inner join [b1-124].M1B_LUP.DBO.MHC_SLS_ORG_RETL u
                            on u.LVL_NO=1 AND u.THERA_ID=774  and t.TerritoryID=u.RETL_TERR_ID
        ...
        rawDbQuery2=""" Select ME_NO,IMSDR_Key,F_NM,L_NM,DrugName,RegionID,PDRP,Matty into #tmp1 FROM  #MMRX_Thera_Name1
                        Where  PDRP='N' AND RegionID NOT IN ('41000','42000')
...
def CompareMattySupraxCapTabcommercialwoManagedMedicaid():
        global ReportOutput,RAWDbOutputSupraxCapTabcommercialwithoutManagedMedicaid
        #Report_Matty()
        RAWDBMattySupraxCapTabcommercialwithoutManagedMedicaid()
        count1 = 0
        print("------------------ReportOutput----------------------",ReportOutput)
        for i in range(RAWDbOutputSupraxCapTabcommercialwithoutManagedMedicaid.__len__()):
            ...
            print("item1 ",item1)
            if(ReportOutput.__contains__(item1)):
                print("passed")
            else:
...
def CompareRAWDBMattyWomensHealthcarecommercialwithoutManagedMedicaid():
    ...
    for i in range(RAWDBMattyWomensHealthcarecommercialwithoutManagedMedicaid.__len__()):
        ...
        ##print("RAWDBMattyWomensHealthcarecommercialwithoutManagedMedicaid",RAWDBMattyWomensHealthcarecommercialwithoutManagedMedicaid)
        print(" ")
        print("item1 ", item1)
        """for j in range(ReportOutput.__len__()):
        ##print(ReportOutput[j])
